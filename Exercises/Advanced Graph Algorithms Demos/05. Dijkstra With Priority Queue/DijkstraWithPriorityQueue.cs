...
namespace _05.Dijkstra_With_Priority_Queue
{
    public class DijkstraWithPriorityQueue
    {
        public static void Main()
        {
            ...
            {
                {nodes[0], new Dictionary<Node, int>()
                { {nodes[6], 10}, {nodes[8], 12} } },
                {nodes[1], new Dictionary<Node, int>()
                { {nodes[4], 20}, {nodes[7], 26}, {nodes[9], 5}, {nodes[11], 6} } },
                {nodes[2], new Dictionary<Node, int>()
                { {nodes[7], 15}, {nodes[8], 14}, {nodes[11], 9} } },
                {nodes[3], new Dictionary<Node, int>()
                    { {nodes[10], 7 } } },
                {nodes[4], new Dictionary<Node, int>()
                { {nodes[1], 20}, {nodes[5], 5}, {nodes[6], 17}, {nodes[11], 11} } },
                {nodes[5], new Dictionary<Node, int>()
                { {nodes[4], 5}, {nodes[6], 6}, {nodes[8], 3}, {nodes[11], 33} } },
                {nodes[6], new Dictionary<Node, int>()
                { {nodes[0], 10}, {nodes[4], 17}, {nodes[5], 6} } },
                {nodes[7], new Dictionary<Node, int>()
                { {nodes[1], 26}, {nodes[2], 15}, {nodes[9], 3}, {nodes[11], 20} }  },
                {nodes[8], new Dictionary<Node, int>()
                { {nodes[0], 12}, {nodes[2], 14}, {nodes[5], 3} } },
                {nodes[9], new Dictionary<Node, int>()
                { {nodes[1], 5}, {nodes[7], 3} } },
                {nodes[10], new Dictionary<Node, int>()
                { {nodes[3], 7 } } },
                {nodes[11], new Dictionary<Node, int>()
                { {nodes[1], 6}, {nodes[2], 9}, {nodes[4], 11}, {nodes[5], 33}, {nodes[7], 20} } }
            };
            ...
            int destinationNode)
        {
            ...
            var path = DijkstraAlgorithm.Dijkstra(graph, nodes, sourceNode, destinationNode);
            if (path == null)
            {
            ...
            else
            {
                ...
                Console.WriteLine($"{formattedPath} (length : {nodes[destinationNode].DistanceFromStart})");
            }
        }
    }
}
