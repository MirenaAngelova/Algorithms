    ...
    public class FastAndFurious2
    {
        ...
        public static void Main()
        {
            ...
            foreach (var car in cars)
            {
                ...
                for (int firstCamera = 0; firstCamera < cameras.Count; firstCamera++)
                {
                    ...
                    {
                        var time = cameras[secondCamera].RecordingTime -
                                   cameras[firstCamera].RecordingTime;
                        var shortestTime = GetShortestTimeBetweenCameras(
                            cameras[secondCamera].CameraName, cameras[firstCamera].CameraName);
                        if (shortestTime < TimeSpan.MaxValue &&
                            !speedingCars.Contains(car.Key) &&
                            time < shortestTime)
                        {
        ...
        private static TimeSpan GetShortestTimeBetweenCameras(string firstCamera, string secondCamera)
        {
            ...
            while (otherCameras.Count > 0)
            {
                ...
                TimeSpan closestCameraDistance = times[closestCamera];
                ...
                {
                    var alternativeTime = times[closestCamera] +
                                          roads[neighbourCamera + "-" + closestCamera].MinimumTime;
            ...
            foreach (var time in times)
            {
                ...
                if (!shortestTimes.ContainsKey(shortestTimeKey) || 
                    time.Value < shortestTimes[shortestTimeKey])
                {
        ...
        private static void ReadInput()
        {
            ...
            while ((line = Console.ReadLine()) != "Records:")
            {
                ...
                var road = new Road(
                    lineParts[0], lineParts[1], double.Parse(lineParts[2]), double.Parse(lineParts[3]));
